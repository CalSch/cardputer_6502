...
#define printf2(...) Serial.printf(__VA_ARGS__)
...
std::string in_text;
int input_head = 0; // for pretending that in_text is a stream
std::string out_text;
std::string list_text;

// fputc replacement for outfil
char fputc_out(char c) {
  out_text.push_back(c);
  Serial.printf("Adding char '%c' (0x%02X)\n",c,c);
}

void fputs_out(const char* str) {
  out_text.append(std::string(str));
  Serial.printf("Adding str '%c'\n",str);
}

char* fgets_in(char* str, int count) {
  if (input_head >= in_text.size())
    return NULL;
  for (int i=0;i<count-1;i++) {
    if (input_head >= in_text.size()) {
      break;
    }
    str[i] = in_text[input_head];
    Serial.printf("Read char '%c' (0x%02X)\n",in_text[input_head],in_text[input_head]);
    input_head++;
    if (str[i]=='\n') break;
  }
  return str;
...
void usage(char* appnam) {
  printf2("Usage: %s [opts] asmfile objfile [lstfile]\n", appnam);
  printf2(" Opts: -d - Output debug info\n");
  printf2("       -h - SREC header record text\n");
  printf2("       -a - Apple-1 monitor format\n");
  printf2("       -p - Commodore PRG format\n");
  printf2("       -s - Motorola SREC format\n");
  printf2("       -x - Intel HEX format");
  printf2("       \n");
  // exit(EXIT_FAILURE);
}
...
void xdump(void) {
  puts("Error Dump");
  oprnd[opridx]=0; printf2("Operand: %s\n", oprnd);
}
...
FILE * opnfil(char* name, char* mode, char* path) {
  if (debug) printf2("Opening file '%s' with mode '%s'\n", name, mode);
  if (debug && strlen(path)) printf2("using path list '%s'\n", path);
  ...
  if (!fp) {
    ...
    while (token != NULL) {
      ...
      token = strtok(NULL, " ");
      if (debug) printf2("token = \"%s\"\n", token);
    }
  }
  if (!fp) xerror("Error Opening File '%s'\n", name);
  if (debug) printf2("File Opened\n");
  return fp;
}
...
FILE * opnobj(char* objnam, char* path) {
  char mode[3] = "w";
  objbin = obin[objtyp];
  if (debug) printf2("Set objbin to %d\n", objbin);  
  objlen = olen[objtyp];
  if (debug) printf2("Set objlen to %d\n", objlen);  
...
struct sym *fndsym(int block, char* name) {
  if (debug) printf2("Searching for Symbol %s in block %d\n", name, block);
  for (int i=0; i < symcnt; i++) {
    if (symtbl[i].block != block || strcmp(symtbl[i].name,name)) continue;
    if (debug) printf2("Found Symbol %s, block %d, size %d, value %04x\n", 
...
void setsym(int value, int bytes) {
  ...
  symbol.refrd = FALSE;
  if (debug) printf2("Set Symbol %s in block %d to value %d, size %d, referred=False\n", 
...
int plabel(void) {
  ...
  if (debug) {
    if (found) printf2("Found Label %s\n", label);
  ...
  if (found && passno == 1) {
    ...
    if (debug) printf2("Initializing Symbol %s\n", label);
    symbol.block = block;
...
int evltrm() {
  ...
  skpspc();
  if (debug) printf2("Term Evaluated to %d\n", result);
  return result;
}
...
int evlopd(int maxsiz) {
  ...
  if (debug) printf2("Operand Evaluated to %d\n", result);
...
void outeot(void) {
  ...
  if (objtyp = INTFIL); fputs_out(":00000001FF\n");
}
...
int lkpopc(struct opc opl[]) {
  if (debug) printf2("Looking up Mnemonic %s\n", mnmnc);
  ...
  for (int i=0; opl[i].name[0]; i++) {
    ...
    token = opl[i].token;
    amode = opl[i].amode;
    if (debug) printf2("Found token %02X, amode %04X\n", token, amode);
    return TRUE;
  }
  return FALSE;
}
...
void asmaln(void) {
  ...
  if (debug) printf2("Aligning to %d Bytes\n", size);
  int fill = size - (curadr % size); if (fill == size) return;
  if (debug) printf2("Filling %d Bytes\n", fill);
...
void asmprc(void) {
  ...
  if (debug) printf2("Ignoring Operand %s\n", oprnd);
}
...
void asmsub(void) {
  ...
  if (debug) printf2("Block Number set to %s\n", oprnd);
}
...
void asmens(void) {
  blknum = 0;
  sprintf(oprnd, "%d", blknum); opridx = strlen(oprnd);
  if (debug) printf2("Block Number reset to %s\n", oprnd);
}
...
void asminf(void) {
  ...
  if (debug) printf2("Include File Set to Name to '%s'\n", incnam);
}
...
int asmpso(int dot) {
  ...
  skpspc();
  if (debug) printf2("Assembling Pseudo-Op %s, Token '%c'\n", mnmnc, token);
  switch (token) {
...
int chkmod(int mode) {
  ...
  if (debug) printf2("Checking Addressing Mode %s, %04X against %04X\n", s, mode, amode);
...
void asmbrn(int setzp) {
  ...
  if (debug) printf2("Assembling Branch Opcode Token 0x%02X\n", token);
  zpage = setzp;
  if (isalpha(*linptr) || *linptr =='.') {
    struct sym *target = evlsym();
  ...
  if (debug) printf2("Calculated Branch Offset of %d\n", offset);
  if ((offset > 127 || offset < -128) && passno == 2) 
    xerror("Branch Out of Range\n", "");
  if (debug) printf2("Branch Offset %d\n", offset);
  opval = offset & 0xFF;
}
...
void asmzpr(void) {
  int bitno = -1;
  if (debug) printf2("Assembling ZeroPage (Relative) Opcode Token 0x%02X\n", token);
...
void asmimd(void) {
  if (debug) printf2("Assembling Immediate Opcode Token 0x%02X\n", token);
  opval = evlopd(0xFF);
  zpage = TRUE;
...
void asmiaz(void) {
  ...
  if (debug) printf2("Assembling Absolute/ZeroPage 0x%02X\n", token);
...
unsigned char fixopc(void) {
  if (debug) printf2("Fixing OpCode $%02X+$%02X\n", token, opmod);
...
void dbgopc(void) {
  printf2("token=$%02X, opmod=$%02X, Address Mode: ", token, opmod);
...
int evlreg(void) {
  ...
  if (pfx && isalpha(*linptr)) return evlhex();
  if (isdigit(*linptr)) return evldec();
  if (!pword(FALSE, name)) xerror("Register Argument Required\n", "");
  for (int i=0; name[i]; i++) if (opridx<MAXSTR) oprnd[opridx++] = name[i];
  if (pfx) pfxstr(pfx, name);
  if (debug) printf2("Parsed Register Name %s \n", name);
...
void asmreg(void){
  if (debug) printf2("Assembling Register Opcode %02x\n", token);
  int regno = evlreg();
  if (debug) printf2("Register Number: %d\n", regno);
  token = token | regno;
  if (amode == ABSLT) {
...
int asmswo(void) {
  ...
  if (debug) printf2("Assembling Sweet 16 Instruction  %s, ", mnmnc);
  if (debug) printf2("Addressing Mode 0x%04X\n", amode);
  skpspc();
  switch (amode) {
  ...
  if (debug) printf2("Writing OpCode $%02X\n", token);
  outbyt(token);                         
  if (debug) printf2("Writing %s Operand %d\n", zpgabs[-zpage], opval);
...
int asmopc(int dot) {
  ...
  if (debug) printf2("Assembling Opcode Token 0x%02X, ", token);
  if (debug) printf2("Addressing Mode Mask 0x%04X\n", amode);
  skpspc();
...
void addsym() {
  ...
  if (debug) printf2("Added symbol %s in block %d to Symbol Table\n", symbol.name, symbol.block);
}


/* Open Include File */
void opninc(void) {
  if (debug) printf2("Opening Include File %s\n", incnam);
  // if (lstfil) fputs("\n", lstfil);
  list_text.append(std::string("\n"));
...
/* Close Include File */
void clsinc(void) {
  if (debug) printf2("Closing Include File %s\n", incnam);
  // if (lstfil) fputs("\n", lstfil);
  list_text.append(std::string("\n"));
  fclose(incfil);
  incfil = NULL;
...
void asmfil(int pass) {
  ...
  if (debug) printf2("Assembling Pass %d\n", pass);
  ...
  if (debug) printf2("Rewinding  Input File\n");
  ...
  while (TRUE) {
    ...
    else linptr = fgets_in(inplin, MAXSTR);
    // if (endasm || linptr == NULL) {if (incfil) {clsinc(); continue;} else break;}
    if (endasm || linptr == NULL) {if (incfil) {clsinc(); continue;} else break;}
    if (debug) printf2("%1d %05d %04X: %s", passno, lineno, curadr, inplin);
    ...
    if (passno == 2) {
      ...
      char str[100]; // TODO: make this safe
      sprintf(str, "%05d %-4s %-9s%-7s %-5s %-16s %s\n", lineno, hexadr, mcode, label, mnmnc, oprnd, cmmnt);
      list_text.append(std::string(str));
  ...
  if (debug) printf2("End address set to %04x\n", endadr);
}
...
int pcoptn(char *argval) {
  ...
  if (debug && objtyp) printf2("Object type set to %d\n", objtyp);
  return TRUE;
}
